 ...
 *
 * Poster precedence: when a save has both a `kind: "poster"` file (a
 * real frame-0 still generated by the main-process ffmpeg pass) and a
 * `kind: "cover"` file (the platform-supplied thumbnail), the
 * generated poster wins. The platform cover is suppressed from
 * standalone slides so the grid card doesn't render it twice.
 */
export function buildMediaUnits(save: Save): MediaUnit[] {
  ...
  // Pair every video with the next available poster (ffmpeg-generated
  // frame 0), falling back to the platform-supplied cover when no
  // generated poster exists yet (e.g. the backfill hasn't run, or
  // ffmpeg is missing). Videos without either still render — the
  ...
  for (let i = 0; i < videos.length; i++) {
    ...
    // Even when a generated poster is in play, consume the paired
    // platform cover so it doesn't reappear as a standalone slide.
